	...
	Drupal.behaviors.runmemo = {
			...
			attach: function (context,settings) {
				...
				$('.page-search-result #block-system-main span.node_check').each(function(){
					...
					$(this).html('<label class="label_check" for="'+nid+'" id="check_'+nid+'"><input type="checkbox" name="id_'+nid+'" id="'+nid+'" value="'+nid+'" class="img_check"/></label>');
				});
				...
				if($('.page-search-result #search-result-cart .summary_selected_photos .placeholder').length > 0) {
					if($('.page-search-result #search-result-cart .summary_selected_photos .placeholder').html() <= 0) {
						// for hide the proceed to checkout link in the search result page
						// when no items in the cart
						$('.page-search-result .content #search-result-cart .proceed_to_cart a').attr('style','display:none');
					}
				}
				// for add to cart functionality when check the checkbox in the search
				// result page
				...
				$('span.node_check input').click(function() {
					...
					if(class_name == 'label_check c_on'){
						...
						$('#check_'+nid).attr('class','label_check');
						remove_from_cart(nid);

					}
					else {
						$(this).parent('label').removeClass('label_check').addClass('label_check c_on');
						$('#check_'+nid).attr('class','label_check c_on');
						add_to_cart(nid);
					}
				...
				// tells whether product with specified nid is in the cart
				...
				function set_total_price() {
					...
					for (var i = 0; i < items.length; i++) {
					    price = price_of_item(items[i]);
					    total_price = total_price + price;
					}
					
					$("span.summary_cost span.placeholder").text(total_price);
				}
				...
				// changes buttons for specified node ID
				...
				$('#add_to_cart').click(function() {
					...
					var class_name = $('#check_'+nid).attr('class');
					if(class_name == 'label_check c_on'){
						$(this).parent('label').removeClass('label_check c_on').addClass('label_check');
